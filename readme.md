...
Hệ thống khóa/mở khóa bằng **pattern lưới 3×3** (giống mở khóa điện thoại) chạy trên kit
...
- Giữ nút **BOOT (USER_BUTTON)** có sẵn trên kit trong **3 giây** → vào trạng thái **đăng ký**
...
- Mở khóa thành công → màn hình hiện **thông báo**, rồi **tự quay về trạng thái khóa sau vài giây**.

### Sản phẩm
1. **Đăng ký pattern** — giữ nút BOOT 3s để vẽ pattern mới, vẽ lại để xác nhận, hệ thống lưu Flash.
2. **Kiểm tra / mở khóa** — vẽ pattern; đúng → mở khóa (xanh) + thông báo; sai → báo lỗi (đỏ) và cho thử lại.
...
| 3 | _(điền)_ | _(điền)_ | _(điền)_ |

---

## 3. MÔI TRƯỜNG HOẠT ĐỘNG

- **Kit:** STM32F429I-DISCO (MCU STM32F429ZIT6, 2MB Flash, màn LCD 240×320 16bpp, cảm ứng STMPE811).
...
2. **Đăng ký:** vẽ pattern (≥ 4 điểm) → màn hiện *"Ve lai de xac nhan"* → vẽ lại đúng như vậy →
   *"Dang luu..."* (màn hình **đứng ~1s** do xóa Flash) → *"Da luu"* → về khóa.
3. **Mở khóa:** vẽ đúng pattern → *"Mo khoa OK"* (xanh) → tự khóa lại sau ~3s. Sai → *"Sai, thu lai"* (đỏ).

> ⚠️ **Về thông báo chữ:** font mặc định của TBS chưa sinh sẵn glyph chữ cái nên **cần mở project
> bằng TouchGFX Designer và Generate Code lại một lần** để các dòng thông báo hiển thị đúng
> (đã khai báo `WildcardCharacters` trong `texts.xml`). Trước khi regenerate, **màu sắc**
> (xanh = đúng, đỏ = sai, cyan = đang vẽ) vẫn phản hồi đầy đủ trạng thái.
